/**
 * KURO Icon Registry — maps app IDs and emoji labels to SVG icons.
 * Use: <KuroIcon name="kuro.chat" size={24} /> or <KuroIcon name="💬" size={24} />
 * Fallback: renders the original emoji string if no SVG mapping exists.
 */
import React from 'react';

// Minimal SVG icon paths (viewBox 0 0 24 24, stroke-based)
const ICON_PATHS = {
...
const APP_MAP = {
  ...
  'kuro.auth': 'auth', 'kuro.admin': 'admin',
};
...
export default function KuroIcon({ name, size = 20, color = 'currentColor', className = '', style = {} }) {
  ...
  if (!path) {
    // Fallback: render as text (emoji or label)
...
// Helper: resolve emoji to icon key (for use in non-React contexts)
