...
// KURO Facilitator — UK fiat rail (scheme: fiat-fps / Faster Payments)
// Same pattern as fiat_sea.cjs. Fails loud until operator creds provisioned.
// Env:
//   KURO_FACILITATOR_RAIL_URL_FIAT_FPS
//   KURO_FACILITATOR_RAIL_CRED_FIAT_FPS
//   KURO_FACILITATOR_RAIL_SECRET_FIAT_FPS  (HMAC, used by verifier)
...
async function settle(payload) {
  ...
  if (!net) {
    return { success: false, network: null, error: `unsupported_uk_scheme:${scheme}` };
  }
