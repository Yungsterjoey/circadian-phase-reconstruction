...
// KURO Facilitator — Australia fiat rail (scheme: fiat-payid / Osko NPP)
// Same pattern as fiat_sea.cjs. Fails loud until operator creds provisioned.
// Env:
//   KURO_FACILITATOR_RAIL_URL_FIAT_PAYID
//   KURO_FACILITATOR_RAIL_CRED_FIAT_PAYID
//   KURO_FACILITATOR_RAIL_SECRET_FIAT_PAYID  (HMAC, used by verifier)

const axios = require('axios');

const SCHEME_TO_NETWORK = {
  'fiat-payid': 'payid-osko',
};
...
async function settle(payload) {
  ...
  if (!net) {
    return { success: false, network: null, error: `unsupported_au_scheme:${scheme}` };
  }
