...
// KURO Facilitator — EU/EEA fiat rail (scheme: fiat-sepa-instant / SCT Inst)
// Same pattern as fiat_sea.cjs. Fails loud until operator creds provisioned.
// Env:
//   KURO_FACILITATOR_RAIL_URL_FIAT_SEPA_INSTANT
//   KURO_FACILITATOR_RAIL_CRED_FIAT_SEPA_INSTANT
//   KURO_FACILITATOR_RAIL_SECRET_FIAT_SEPA_INSTANT  (HMAC, used by verifier)

const axios = require('axios');

const SCHEME_TO_NETWORK = {
  'fiat-sepa-instant': 'sepa-instant',
};
...
async function settle(payload) {
  ...
  if (!net) {
    return { success: false, network: null, error: `unsupported_eu_scheme:${scheme}` };
  }
