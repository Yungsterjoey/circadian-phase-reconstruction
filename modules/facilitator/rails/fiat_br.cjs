...
// KURO Facilitator — Brazil fiat rail (scheme: fiat-pix / BCB PIX)
// Same pattern as fiat_sea.cjs. Fails loud until operator creds provisioned.
// Env:
//   KURO_FACILITATOR_RAIL_URL_FIAT_PIX
//   KURO_FACILITATOR_RAIL_CRED_FIAT_PIX
//   KURO_FACILITATOR_RAIL_SECRET_FIAT_PIX  (HMAC, used by verifier)
...
async function settle(payload) {
  ...
  if (!net) {
    return { success: false, network: null, error: `unsupported_br_scheme:${scheme}` };
  }
